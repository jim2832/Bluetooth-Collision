												...
												for(int dev=0; dev < arrNum.length ; dev++ ) {
													for(int u=0; u < badChannel.size(); u++) {
														 if(deviceSeq.get(dev) == badChannel.get(u)) {
															...
															for(int m=0; m < normalChannel.size();m++) {
																if(normalChannel.size() < 40) {
																	int normalNum = 0;
																	...
																	int badNum = 0;
																	badNum = rand_channel.nextInt(badChannel.size());
																	deviceSeq.set(dev,badChannel.get(badNum));
																}
															}
															if(deviceSeq.get(dev)-min == 0 || deviceSeq.get(dev)-min < 0) {
																deviceSeq.set(dev,deviceSeq.get(dev)+min);
															}
															else {
																deviceSeq.set(dev,deviceSeq.get(dev)-min);
															}
